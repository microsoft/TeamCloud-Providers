...
namespace TeamCloud.Providers.GitHub.Orchestrations
{
    public static class TeamCloudUserUpdateOrchestration
    {
            ...
            ILogger log)
        {
            ...
            var command = functionContext.GetInput<ProviderTeamCloudUserUpdateCommand>();
            ...
            using (log.BeginCommandScope(command))
            {
                try
                {
                    functionContext.SetCustomStatus("Updating user", commandLog);

                    await functionContext
                        .CallActivityWithRetryAsync(nameof(TeamCloudUserUpdateActivity), command.Payload)
                        .ConfigureAwait(true);
