...
namespace TeamCloud.Providers.GitHub.Orchestrations
{
    public static class TeamCloudUserCreateOrchestration
    {
            ...
            ILogger log)
        {
            ...
            var command = functionContext.GetInput<ProviderTeamCloudUserCreateCommand>();
            ...
            using (log.BeginCommandScope(command))
            {
                try
                {
                    functionContext.SetCustomStatus("Creating user", commandLog);

                    await functionContext
                        .CallActivityWithRetryAsync(nameof(TeamCloudUserCreateActivity), command.Payload)
                        .ConfigureAwait(true);
