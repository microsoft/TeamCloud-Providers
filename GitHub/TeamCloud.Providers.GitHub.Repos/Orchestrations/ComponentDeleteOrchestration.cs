...
{
    public static class ComponentDeleteOrchestration
    {
        [FunctionName(nameof(ComponentDeleteOrchestration))]
            ...
            ILogger log)
        {
            ...
            var command = (ProviderComponentDeleteCommand)commandContext.Command;
            ...
            using (log.BeginCommandScope(command))
            {
                try
                {
                    functionContext.SetCustomStatus("Deleting component", commandLog);

                    commandResult.Result = await functionContext
                        .DeleteComponentAsync(command, command.Payload)
                        .ConfigureAwait(true);
                }
                catch (Exception exc)
                {
